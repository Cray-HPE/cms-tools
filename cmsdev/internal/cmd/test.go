...
import (
	...
	"stash.us.cray.com/cms-tools/cmsdev/internal/test/ipxe"
	"stash.us.cray.com/cms-tools/cmsdev/internal/test/tftp"
...
func RunCTTest(service, stage string, local, smoke, ct bool) bool {
	switch service {
		case "bos":		return bos.IsBOSRunning(local, smoke, ct, stage)
		case "cfs":		return cfs.IsCFSRunning(local, smoke, ct, stage)
		case "conman":	return con.IsConmanRunning(local, smoke, ct, stage)
		case "ims":		return ims.IsIMSRunning(local, smoke, ct, stage)
		case "ipxe":	return ipxe.IsIPXERunning(local, smoke, ct, stage)
		case "tftp":	return tftp.IsTFTPRunning(local, smoke, ct, stage)
		case "vcs", "gitea":	
...
cmsdev test cfs --smoke --verbose
  # runs cfs smoke tests with verbosity`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		case "ipxe":
			if ct || smoke { 
				_ = ipxe.IsIPXERunning(local, smoke, ct, stage)
			} else { // api
				common.Usagef("no api tests currently found for %s\n", service)
			}
		case "tftp":
			if ct || smoke { 
				_ = tftp.IsTFTPRunning(local, smoke, ct, stage)
			} else { // api
				common.Usagef("no api tests currently found for %s\n", service)
			}
		case "vcs", "gitea":
...
func init() {
	...
	testCmd.Flags().BoolP("local", "l", false, "run tests locally, not implemeted")
